...
@Service
public class MissionChercheurServiceImpl extends AbstractServiceImpl<Mission> implements MissionChercheurService {
...
private MissionDao missionDao;
    ...
    private ArchivableService<Mission> archivableService;
        ...
        @Autowired
        private PieceJointeMissionChercheurService pieceJointeMissionService ;
...
return  missionDao.save(mission);
    ...
    private void prepareSave(Mission mission){
        mission.setDateCreation(new Date());
            ...
            mission.setUsername(currentUser.getUsername());
            }
...
missions.forEach(e->missionDao.save(e));
        ...
        public List<Mission> findAllNonArchive(){
        String query = "SELECT o FROM Mission o  WHERE o.archive != true AND o.visible = true";
        return entityManager.createQuery(query).getResultList();
        }

        public List<Mission> findAllByOwner(){
        List<Mission> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM Mission o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
