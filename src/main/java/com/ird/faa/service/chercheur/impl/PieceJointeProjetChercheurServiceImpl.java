...
@Service
public class PieceJointeProjetChercheurServiceImpl extends AbstractServiceImpl<PieceJointeProjet> implements PieceJointeProjetChercheurService {
...
public List<PieceJointeProjet> findAll(){
    List<PieceJointeProjet> resultat= new ArrayList();
...
return  pieceJointeProjetDao.save(pieceJointeProjet);
    ...
    private void prepareSave(PieceJointeProjet pieceJointeProjet){
        pieceJointeProjet.setDateCreation(new Date());
            ...
            pieceJointeProjet.setUsername(currentUser.getUsername());
            }
...
pieceJointeProjets.forEach(e->pieceJointeProjetDao.save(e));
        ...
        public List<PieceJointeProjet> findAllNonArchive(){
        String query = "SELECT o FROM PieceJointeProjet o  WHERE o.archive != true AND o.visible = true";
        return entityManager.createQuery(query).getResultList();
        }

        public List<PieceJointeProjet> findAllByOwner(){
        List<PieceJointeProjet> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM PieceJointeProjet o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
