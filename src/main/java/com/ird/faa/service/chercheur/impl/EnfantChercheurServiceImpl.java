...
@Service
public class EnfantChercheurServiceImpl extends AbstractServiceImpl<Enfant> implements EnfantChercheurService {
...
public List<Enfant> findAll(){
    List<Enfant> resultat= new ArrayList();
...
return  enfantDao.save(enfant);
    ...
    private void prepareSave(Enfant enfant){
        enfant.setDateCreation(new Date());
            ...
            enfant.setUsername(currentUser.getUsername());
            }
...
enfants.forEach(e->enfantDao.save(e));
        ...
        public List<Enfant> findAllNonArchive(){
        String query = "SELECT o FROM Enfant o  WHERE o.archive != true AND o.visible = true";
        return entityManager.createQuery(query).getResultList();
        }

        public List<Enfant> findAllByOwner(){
        List<Enfant> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM Enfant o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
