...
private ReclamationDao reclamationDao;
    ...
    private ArchivableService<Reclamation> archivableService;
        ...
        @Autowired
        private PieceJointeReclamationChercheurService pieceJointeReclamationService ;
        @Autowired
...
return  reclamationDao.save(reclamation);
    ...
    private void prepareSave(Reclamation reclamation){
        reclamation.setDateCreation(new Date());
            ...
            reclamation.setUsername(currentUser.getUsername());
            }
...
reclamations.forEach(e->reclamationDao.save(e));
        ...
        public List<Reclamation> findAllNonArchive(){
        String query = "SELECT o FROM Reclamation o  WHERE o.archive != true AND o.visible = true";
            query+= " ORDER BY o.dateReclamation";
        return entityManager.createQuery(query).getResultList();
        }

        public List<Reclamation> findAllByOwner(){
        List<Reclamation> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM Reclamation o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
            query+= " ORDER BY o.dateReclamation";
