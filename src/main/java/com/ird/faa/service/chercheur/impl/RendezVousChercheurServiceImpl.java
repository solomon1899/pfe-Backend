...
@Service
public class RendezVousChercheurServiceImpl extends AbstractServiceImpl<RendezVous> implements RendezVousChercheurService {
...
private RendezVousDao rendezVousDao;
    ...
    private ArchivableService<RendezVous> archivableService;
        @Autowired
        private PieceJointeRendezVousChercheurService pieceJointeRendezVousService ;
...
return  rendezVousDao.save(rendezVous);
    ...
    private void prepareSave(RendezVous rendezVous){
        rendezVous.setDateCreation(new Date());
            ...
            rendezVous.setUsername(currentUser.getUsername());
            }
...
rendezVouss.forEach(e->rendezVousDao.save(e));
        ...
        public List<RendezVous> findAllNonArchive(){
        String query = "SELECT o FROM RendezVous o  WHERE o.archive != true AND o.visible = true";
        return entityManager.createQuery(query).getResultList();
        }

        public List<RendezVous> findAllByOwner(){
        List<RendezVous> result= new ArrayList();
        User currentUser = SecurityUtil.getCurrentUser();
        if (currentUser != null && StringUtil.isNotEmpty(currentUser.getUsername())){
        String query = "SELECT o FROM RendezVous o  WHERE o.archive != true AND o.visible = false AND o.username = '"+ currentUser.getUsername()+"'";
